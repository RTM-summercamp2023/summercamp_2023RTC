...
from PyQt5.QtWidgets import QMainWindow,QApplication, QWidget, QVBoxLayout, QPushButton,QVBoxLayout, QGroupBox, QRadioButton,QLabel
...
guiaruco_spec = ["implementation_id", "GUIaruco", 
         "type_name",         "GUIaruco", 
...
class gui(QMainWindow,Ui_MainWindow):
   def __init__(self, parent=None):
        ...
        self.setupUi(self)
        self.button1 = QPushButton("aruco1")
        self.button2 = QPushButton("aruco2")


   def start_process1(self):  #HOMEボタンを押したときに呼び出される
       global global_var
       with global_var.get_lock():
           global_var.value = 1
       print("Home")

   def start_process2(self):  #Reception deskボタンを押したときに呼び出される
       global global_var
       with global_var.get_lock():
           global_var.value = 2
...
if __name__ == "__main__":
    #並列処理の対象を指定
    process_a = multiprocessing.Process(target=main)
    process_b = multiprocessing.Process(target=main2)
    #プロセススタート
    process_a.start()
    process_b.start()
    #プロセスの終了を待機
